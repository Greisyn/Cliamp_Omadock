...
// oShelf-style action pill: transparent idle, foreground tint on hover,
// accent tint + accent ink when selected. Ink always comes from the theme.
Rectangle {
  id: root
  ...
  Row {
    ...
    DockGlyph { visible: !!root.icon; kind: root.icon; width: root.icon ? 20 : 0; height: 20; ink: root.selected ? Color.accent : Color.foreground }
    Text {
      id: caption
      visible: root.label !== ""
